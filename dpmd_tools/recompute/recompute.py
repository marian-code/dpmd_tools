...
class Recompute:
    ...
    # * prepare job ********************************************************************
    def _prepare_calc(self, index: int, server: str, atoms: Atoms) -> Tuple[Path, str]:
        ...
        if self.INCAR_TEMPLATE:
            ...
            job_script = Dispatcher.get(server)(
                n_nodes, job_name, self.SCAN, priority=True, hour_length=12
            )
    ...
    def _submit2server(self, server: str, free_slots: int, total_jobs: int):
        ...
        for _ in range(free_slots):
            ...
            prepare_dir, job_name = self._prepare_calc(
                job.index, server, job.pop("atoms")
            )
