...
def main():
    ...
    with Connection(args["server"], local=False, quiet=True) as c:
        ...
        dirs_mapping: List[Tuple[Path, SSHPath]] = []
        for d in dirs:
            data = d / "deepmd_data/for_train"
            if data.is_dir():
                dirs_mapping.append((data, target_dir / d.name))
            else:
                print(f"could not find data in {data}")

        print("upload summary:")
        print("------------------------------------------------------------------")
        for src, dst in dirs_mapping:
